...
void run_admin_command(const std::string& cmdLine, AdminContext& ctx)
{
	...
	if (cmd == "help" || cmd == "?") {
		LOG_INFO("commands: help status players kick <peer|key> [ban] say TEXT save quit maxplayers N reload motd TEXT pvp on|off password TEXT ban|unban|bans");
	} else if (cmd == "status") {
		LOG_INFO("version=%s git=%s", kServerVersion, CMP_GIT_VERSION);
		LOG_INFO("status name=%s port=%u clients=%zu/%d host=%u interest=%.0f pvp=%s modHash=0x%08X datagrams=%llu bad=%llu",
			cfg.name.c_str(), static_cast<unsigned>(cfg.port), clients.size(), cfg.maxPlayers, world.hostPeerId,
			cfg.interestUu, cfg.pvp ? "on" : "off", r.sessionModHash,
			static_cast<unsigned long long>(r.datagrams), static_cast<unsigned long long>(r.badHeaders));
		for (const auto& [peerId, c] : clients) {
			LOG_INFO("  peer=%u key=%s name=%s tcp=%s udp=%s lastSeen=%.1f poses=%llu appear=%zu inv=%zu",
				c.peerId, c.playerKey.c_str(), c.name.c_str(), addr_key(c.tcpAddr).c_str(),
				c.udpBound ? addr_key(c.udpAddr).c_str() : "-", t - c.lastSeen,
		...
		for (const auto& [peerId, c] : clients) {
			LOG_INFO("player peer=%u key=%s name=%s tcp=%s poses=%llu last=%.1fs",
				c.peerId, c.playerKey.c_str(), c.name.c_str(), addr_key(c.tcpAddr).c_str(),
		...
		for (auto it = clients.begin(); it != clients.end();) {
			...
			r.send_tcp(it->second, &bye, sizeof(bye), "ByeKick");
			...
			r.unbind_udp(it->second);
			r.close_conn(it->second.connId);
			it->second.connId = 0;
			it = clients.erase(it);
		...
		for (auto& [_, c] : clients) r.send_tcp(c, &chat, sizeof(chat), "Chat");
